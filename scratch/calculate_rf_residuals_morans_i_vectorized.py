...
def main():
    ...
    static_img = ee.Image.cat([dem.rename("elevation"), slope, dist_to_river, dist_to_coast, jrc_occ.rename("occurrence")])
    
    print("Sampling static terrain features and JRC occurrence at points...")
    ...
    for idx, f in enumerate(features):
        ...
        d_riv = props.get('dist_to_river', 5000.0)
        d_cst = props.get('dist_to_coast', 10000.0)
        
        # Clean NaNs
        elev = 0.0 if elev is None else elev
        slp = 0.0 if slp is None else slp
        d_riv = 5000.0 if d_riv is None else d_riv
        d_cst = 10000.0 if d_cst is None else d_cst
        
        X_static.append([elev, slp, d_riv, d_cst])
    ...
    # Calculate Moran's I
    print("Running Moran's I calculations and permutations...")
